...
def _delphi_get(
    ...
    resp = _get_session().get(
        f"{_DELPHI_BASE}/{endpoint}/", params=params, timeout=60
    )
    resp.raise_for_status()
    body = resp.json()
    ...
    if body.get("result") not in (1, 2):
        raise ValueError(
            f"Delphi {endpoint} API error: {body.get('message', 'unknown')}"
        )
...
def flusurv_fetch(
    ...
    """
    cache_key = f"flusurv:{','.join(sorted(l.lower() for l in locations))}:{epiweeks}"
    return _delphi_get(
        "flusurv",
        {"locations": ",".join(locations), "epiweeks": epiweeks},
        cache_key,
    )
...
def fluview_fetch(
    ...
    return _delphi_get(
        "fluview",
        {"regions": ",".join(regions), "epiweeks": epiweeks},
        cache_key,
    )


def fluview_clinical_fetch(
    regions: list[str],
    epiweeks: str,
) -> list[dict[str, Any]]:
