...
Example:
    from grasp.sdk import get_hantavirus_cases, summarize_hantavirus_by_year
    cases = get_hantavirus_cases(outcome="Dead")

    from grasp.sdk import get_fluview_ili, get_fluview_clinical
    ili = get_fluview_ili(regions=["nat", "ca"], epiweeks="202001-202026")
    lab = get_fluview_clinical(regions=["nat"], epiweeks="202001-202026")

    from grasp.sdk import get_flusurv_net, summarize_flusurv_by_season
...
from grasp.client import clear_cache as _clear_cache
from grasp.client import fetch, fluview_clinical_fetch, fluview_fetch, flusurv_fetch
from grasp.datasets import DATASETS, FLUSURV_LOCATIONS, STATE_FIPS
...
def get_hantavirus_cases(
    ...
    if year is not None:
        year_str = str(year)
        records = [r for r in records if _parse_year(r.get("IllnessOnsetDate", "")) == year_str]
...
def summarize_fluview_ili_by_region(
    ...
    all_regions = (
        ["nat"]
        + [f"hhs{i}" for i in range(1, 11)]
        + [f"cen{i}" for i in range(1, 10)]
    )
