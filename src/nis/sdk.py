...
Examples
--------
    from nis.sdk import list_years, stream_records, get_vaccination_rates

    # Available years for each survey
    print(list_years("child"))

    # Stream raw respondent records (dicts) — no storage
    for rec in stream_records("child", 2022, state_fips="06"):
...
from nis.parser import parse_sas_columns, stream_dat
...
def get_vaccination_rates(
    ...
    for fips, recs in sorted(buckets.items()):
        ...
        row["hesitancy_pct"] = (
            round(100 * sum(hesi_vals) / len(hesi_vals), 1) if hesi_vals else None
        )
...
def get_national_rates(
    ...
    total_hesi_d = sum(
        r["n_respondents"] for r in state_rows if r.get("hesitancy_pct") is not None
    )
    national["hesitancy_pct"] = (
        round(100 * total_hesi_n / total_hesi_d, 1) if total_hesi_d else None
    )
