...
def is_wonder_html(url: str) -> bool:
    ...
    if not (u.scheme and u.netloc):
        return False
    return u.netloc.lower() == urlparse(
        BASE
    ).netloc.lower() and u.path.lower().endswith(".html")
...
def pick_best_html(candidates: list[str], fallback: str | None = None) -> str:
    ...
    filtered = [
        u for u in candidates if not any(p in u.lower() for p in ignore_patterns)
    ]
    if not filtered:
...
def probe_d_id(
    dnum: int, session: requests.Session | None = None, delay_sec: float = 0.25
) -> dict:
    sid = f"D{dnum}"
    ...
    try:
        ...
        chosen = pick_best_html(
            dedup, fallback=r.url if is_wonder_html(r.url) else None
        )
        ...
        if chosen:
            ...
            row["discovery"] = (
                "redirect" if chosen.rstrip("/") != ctrl.rstrip("/") else "direct"
            )
            ...
            row["discovery"] = (
                "direct" if r.status_code == 200 else f"http_{r.status_code}"
            )
